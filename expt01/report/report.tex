...
\end{multicols}
\section{Theory}
The experiment is based on the measurement of a geometric phase which is independent of the optical path length and is solely based on the geometry of the evolution of the electromagnetic wave. We will discuss about \textit{Pancharatnam-Berry phase}, wherein the wave propagates in a fixed direction but undergoes a continuous change in the state of polarisation, which is represented by a closed loop in the Poincar\'e sphere.
\begin{figure}[H]
  \centering
  \input{poincare.tex}
